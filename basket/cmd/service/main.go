...
import (
	...
	"github.com/Chengxufeng1994/event-driven-arch-in-go/basket/migrations"
...
func run() (err error) {
	...
	s, err := system.NewSystem("MALL BOTS", "mallbots-baskets", cfg, log)
	if err != nil {
	...
	if err = basket.Root(s.Waiter().Context(), s); err != nil {
		return err
	}

	log.Info("started baskets service")
	defer log.Info("stopped baskets service")
