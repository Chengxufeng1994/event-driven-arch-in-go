...
describe('Baskets UI', () => {
  ...
  context('calling startBasket', () => {
    describe('with a customer ID', () => {
      ...
      it('should start a new basket for the customer', () => {
        return provider.executeTest((mockServer) => {
          ...
          return client.startBasket('customer-id').then((response) => {
    ...
    describe('without any customer ID', () => {
      before(() => {
        provider
          .uponReceiving('a request to start a basket without a customerId')
          .withRequest({
            method: 'POST',
            path: '/api/v1/baskets',
            body: {
      ...
      it('should not start a new basket for the customer', async () => {
        return provider.executeTest((mockServer) => {
          const client = new Client(mockServer.url);
          return client.startBasket('').catch(({ response }) => {
    ...
    describe('with a valid product and quantity', () => {
      before(() => {
        ...
        provider
          ...
          .withRequest({
            method: 'PUT',
            path: `/api/v1/baskets/${basketId}/addItem`,
            body: {
              productId: productId,
              quantity: 1,
            },
      ...
      it('should add the item to the basket', () => {
        return provider.executeTest((mockServer) => {
          const client = new Client(mockServer.url);
          return client.addItem(basketId, productId, 1).then((response) => {
