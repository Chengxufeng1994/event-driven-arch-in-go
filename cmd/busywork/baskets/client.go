...
import (
	...
	"github.com/Chengxufeng1994/event-driven-arch-in-go/basket/basketsclient"
	"github.com/Chengxufeng1994/event-driven-arch-in-go/basket/basketsclient/basket"
	"github.com/Chengxufeng1994/event-driven-arch-in-go/basket/basketsclient/item"
...
func (c *client) StartBasket(ctx context.Context, customerID string) (string, error) {
	resp, err := c.c.Basket.StartBasket(&basket.StartBasketParams{
		Body:    &models.V1StartBasketRequest{CustomerID: customerID},
		Context: ctx,
	})
	if err != nil {
		return "", err
...
func (c *client) CheckoutBasket(ctx context.Context, basketID, paymentID string) error {
	_, err := c.c.Basket.CheckoutBasket(&basket.CheckoutBasketParams{
		Body:    &models.BasketServiceCheckoutBasketBody{PaymentID: paymentID},
		ID:      basketID,
		Context: ctx,
	})
	return err
