...
import (
	...
	"github.com/Chengxufeng1994/event-driven-arch-in-go/depot/internal/application/service"
...
func (m *Module) PrepareRun(ctx context.Context, mono monolith.Monolith) error {
	...
	application := logging.NewLoggingApplicationAccess(
		service.NewShoppingListApplication(shoppingListRepository, grpcStoreClient, grpcProductClient, grpcOrderClient),
		mono.Logger(),
	)

	if err := grpcv1.Register(ctx, application, mono.RPC().GRPCServer()); err != nil {
		return err
	}
