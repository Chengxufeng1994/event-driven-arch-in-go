...
import (
	...
	"github.com/Chengxufeng1994/event-driven-arch-in-go/notification/migrations"
)
...
func run() (err error) {
	...
	s, err := system.NewSystem("MALL BOTS", "mallbots-notifications", cfg, log)
	if err != nil {
	...
	if err = notification.Root(s.Waiter().Context(), s); err != nil {
		return err
	}

	log.Info("started notifications service")
	defer log.Info("stopped notifications service")
