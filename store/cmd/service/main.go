...
import (
	...
	"github.com/Chengxufeng1994/event-driven-arch-in-go/store/migrations"
)
...
func run() (err error) {
	...
	s, err := system.NewSystem("MALL BOTS", "mallbots-stores", cfg, log)
	if err != nil {
	...
	if err = store.Root(s.Waiter().Context(), s); err != nil {
		return err
	}

	log.Info("started stores service")
	defer log.Info("stopped stores service")
