...
import (
	...
	"github.com/Chengxufeng1994/event-driven-arch-in-go/store/storesclient"
...
type storesFeature struct {
	db     *sql.DB
	client *storesclient.APIStoreV1EventProto
}
...
func (c *storesFeature) iCreateTheStoreCalled(ctx context.Context, name string) context.Context {
	resp, err := c.client.Store.CreateStore(store.NewCreateStoreParams().WithBody(&models.V1CreateStoreRequest{
		Location: "anywhere",
		Name:     name,
	}))
...
func (c *storesFeature) iCreateTheProductCalledWithPrice(ctx context.Context, name string, price float64) (context.Context, error) {
	...
	resp, err := c.client.Product.AddProduct(product.NewAddProductParams().WithStoreID(storeID).WithBody(&models.StoresServiceAddProductBody{
		Name:  name,
		Price: price,
	}))
	ctx = setLastResponseAndError(ctx, resp, err)
